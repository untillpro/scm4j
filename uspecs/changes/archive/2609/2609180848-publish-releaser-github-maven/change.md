...
- Treat tags named `scm4j-releaser/<version>` as release boundaries; the publishing workflow derives the Maven version from the matching tag and publishes the tagged commit only, consistent with [component-aware release tag naming](../../../../../scm4j-releaser/src/main/java/org/scm4j/releaser/Utils.java) and its [tag naming coverage](../../../../../scm4j-releaser/src/test/java/org/scm4j/releaser/UtilsTest.java).
...
- Releases configure `scm4j-releaser` as a subfolder-backed component named `scm4j-releaser`, so the releaser-generated tag is `scm4j-releaser/<version>`, and each version suffix is a valid Maven version.
...
- [x] create: [.github/workflows/publish-releaser.yml](../../../../../.github/workflows/publish-releaser.yml) (manual edit; no applicable GitHub CLI command generates a repository workflow)
  - trigger only for pushed tags matching `scm4j-releaser/*`, derive `releaserVersion` by removing the `scm4j-releaser/` prefix from `github.ref_name`, and reject an empty or invalid Maven version before invoking Gradle
